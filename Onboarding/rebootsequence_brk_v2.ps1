...
$s_creds = Get-StoredCredential -Target Reboot
$creds = Get-StoredCredential -Target Reboot -AsCredentialObject
...
while($result -ne 0){
    ...
    $scheduledtask = Invoke-Command -ComputerName $server -ScriptBlock {
    $(Get-ScheduledTask  | Where-Object TaskName -eq PSWindowsUpdate)
    } -Credential $credential -Verbose

    if ($scheduledtask.State -notlike "Ready"){
        $result = $result + 4
        "   (WARN) Job läuft noch oder nicht gefunden" | out-file -filepath $logpath -append
    }else{"   (INFO) Kein Job aktiv" | out-file -filepath $logpath -append}
    
    $dayofreboot   = $(Get-WUSettings -ComputerName $server).ScheduledInstallDay.Substring(0,1)
   ...
   $result = 999} #Keine Assets fuer den heutigen Tag in der Input-Datei gefunden
    ...
    ForEach($PSItem in $datareboot) {
        ...
        $restartcmd = "Restart-Computer -ComputerName " + $restartgrp + " -Wait -For WinRM -Delay 2 -Force -Timeout 1800"
        ...
        $uptime = $(Invoke-Command -ComputerName $server -ScriptBlock {(Get-Date) - (Get-CimInstance Win32_OperatingSystem).LastBootupTime} -Credential $credential).totalminutes
        if($uptime -gt 10){
            $rebootresult = 1
        }
