...
# Install MSSQL Express 2022
$URL = "https://download.microsoft.com/download/5/1/4/5145fe04-4d30-4b85-b0d1-39533663a2f1/SQL2022-SSEI-Expr.exe"
Set-Location $TempDir
Start-BitsTransfer $URL $TempDir -RetryInterval 60 -RetryTimeout 180 -ErrorVariable err
.\SQL2022-SSEI-Expr.exe /Action=Download /MediaType=Core /Quiet
Invoke-Expression "$env:USERPROFILE\Downloads\SQLEXPR_x64_DEU.exe /FEATURES='SQL, Tools' /QS /IACCEPTSQLSERVERLICENSETERMS /ADDCURRENTUSERASSQLADMIN=1 /ACTION='Install' /ERRORREPORTING=0 /INSTANCENAME='SQLEXPRESS'"

# Install SSMS 20.1
$URL = "https://aka.ms/ssmsfullsetup?clcid=0x407"
Invoke-WebRequest -Uri  $URL -OutFile "C:\mr_managed_it\Temp\SSMS-Setup-DE.exe"
Invoke-Expression ".\SSMS-Setup-DE.exe /install /quiet / norestart"

# Install WSUS (WSUS Services, Management tools)
$WSUS_INSTALL = Install-WindowsFeature -Name UpdateServices-Services,UpdateServices-DB -IncludeManagementTools
...
.\wsusutil.exe postinstall SQL_INSTANCE_NAME="%COMPUTERNAME%\SQLEXPRESS" CONTENT_DIR="W:\WSUS_Updates"
...
$ConnectionString = 'server=localhost\SQLEXPRESS;database=SUSDB;trusted_connection=true;'
...
$ConnectionString = 'server=localhost\SQLXPRESS;database=SUSDB;trusted_connection=true;'
$SQLConnection= New-Object System.Data.SQLClient.SQLConnection($ConnectionString)
$SQLConnection.Open()
$SQLCommand = $SQLConnection.CreateCommand()
...
Register-ScheduledTask -Action $action -Trigger $trigger -User $credentials.UserName -Password $credentials.GetNetworkCredential().Password -TaskName "MR Active Patching Sunday" -Description "PSWindowsUpdate Sonntag2"
...
Write-Host "Irgendetwas ist bei der Installation der Server-Rolle WSUS schiefgegangen. Die Installation wird abgebrochen."
}
